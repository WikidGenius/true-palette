(function(){
  ...
  function setSettingsOpen(open){
    ...
    if(label)label.textContent=open?'Hide camera':'Camera';
  }
  ...
  function rewritePrompt(){
    ...
    prompt.innerHTML=`<span class="tinter-question-label">${label}</span><span class="tinter-question-main">Which color makes your skin look more even and your eyes and lips stand out naturally?</span>${step?`<span class="tinter-step-count">${step}</span>`:''}`;
  }
  ...
  function installModalImprovements(){
    ...
    if(!q('#tinterSettings')){
      ...
      settings.innerHTML='<span class="tinter-settings-icon" aria-hidden="true">⚙︎</span><span class="tinter-settings-label">Camera</span>';
    ...
    if(!q('.tinter-judge-line')){
      prompt.insertAdjacentHTML('afterend','<p class="tinter-judge-line">Watch your face—not the swatches. Look for more even-looking skin, softer shadows, and eyes or lips that stand out naturally.</p>');
    }
