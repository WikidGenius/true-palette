...
export function createCameraController({state,ui,isCurrentSession}){
  ...
  function autoBalance(){
    const video=ui.getVideo();
    if(!video?.videoWidth){ui.setBalanceNote('The camera is still starting. Try again.');return}
    try{
      ...
      for(let index=0;index<pixels.length;index+=4){red+=pixels[index];green+=pixels[index+1];blue+=pixels[index+2];count++}
      ...
      ui.setBalanceNote(strength<25?'White already looks close to neutral.':strength<70?'Auto balance applied.':'A strong color cast remains. Neutral daylight will be more reliable.');
    }catch{
  ...
  function sampleLighting(video){
    ...
    for(let index=0;index<pixels.length;index+=4){red+=pixels[index];green+=pixels[index+1];blue+=pixels[index+2];count++}
    red/=count;green/=count;blue/=count;
    return {brightness:.2126*red+.7152*green+.0722*blue,cast:Math.max(red,green,blue)-Math.min(red,green,blue)};
  }
  ...
  function classifyLighting(samples){
    ...
    let key='good',message='Lighting looks good.';
    if(brightness<45){key='low';message='The image is too dark. Face a window or add soft light.'}
    else if(brightness>230){key='low';message='The image is too bright. Move away from direct light.'}
    else if(cast>75){key='low';message='The room light is strongly colored. Neutral daylight will be more reliable.'}
    else if(brightness<70||brightness>210||cast>48){key='okay';message='Lighting is usable, but neutral daylight would improve confidence.'}
  ...
  async function checkLighting(session=state.sessionId){
    ...
    try{
      ...
      if(!isCurrentSession(session))return;
      const quality={key:'okay',message:'Lighting could not be measured. You can continue, but confidence will be reduced.',brightness:null,cast:null};
      state.lightingQuality=quality;
  ...
  async function start(session=state.sessionId){
    ...
    try{
      ...
      const stream=await navigator.mediaDevices.getUserMedia({
        video:{facingMode:'user',width:{ideal:720},height:{ideal:960},frameRate:{ideal:30,max:30}},audio:false
      });
      window.clearTimeout(permissionTimer);permissionTimer=0;
      if(!isCurrentSession(session)){stream.getTracks().forEach(track=>track.stop());return}
      state.stream=stream;
      const video=ui.getVideo();
  ...
  return {start,stop,checkLighting,applyBalance,resetBalance,autoBalance};
}
