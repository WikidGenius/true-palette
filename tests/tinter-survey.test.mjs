...
assert.deepEqual({...Survey.scoreResponse(initial[0],'left','good').delta},zeros());
...
assert.deepEqual({...Survey.scoreResponse(validation[0],'left','good').delta},zeros());

const quality=Survey.reliability([
...
assert.equal(quality.calibrationScore,1);
assert.equal(quality.rushedRate,0);
const stats=Survey.axisStats([],scores,evidence);
assert.ok(Survey.confidenceSummary(stats,{calibrationScore:.5,rushedRate:0,sideBias:0,contradictionRate:0},{accuracy:1}).score<=.74);
assert.ok(Survey.compareSessions({temp:10,value:-20,chroma:5,def:-5,hue:8},{temp:12,value:-19,chroma:4,def:-4,hue:7}).percent>90);
